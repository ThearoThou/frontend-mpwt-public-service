<script setup lang="ts">
  ...
  function retryVehicle () {
    if (loading.value) return

    void loadVehicle()
  }

  function retryInspectionHistory () {
    if (inspectionHistoryLoading.value) return
...
<template>
  <section class="citizen-auth-type-scale vehicle-details-view">
    ...
    <v-alert v-else-if="loadError || !vehicle" type="error" variant="tonal"><div class="d-flex flex-wrap align-center justify-space-between ga-3">{{ $t('inspection_vehicle_detail_load_error') }}<v-btn
      color="error"
      :disabled="loading"
      :loading="loading"
      variant="outlined"
    ...
    <template v-else>
      ...
      <div class="details-summary-grid">
        ...
        <aside class="inspection-status-card"><div class="d-flex align-center justify-space-between ga-2"><h2>{{ $t('inspection_status') }}</h2>
                                                ...
                                                <v-chip
                                                  :class="['details-inspection-status', `details-inspection-status--${inspectionState(vehicle)}`]"
                                                  :color="stateColor(vehicle)"
                                                  :prepend-icon="stateIcon(vehicle)"
                                                  size="small"
                                                  variant="flat"
                                                >{{ stateText }}</v-chip></div>

          <dl><div><dt>{{ $t('inspection_last_inspection_date') }}</dt><dd>{{ formatDate(vehicle.lastInspectionDate) }}</dd></div><div><dt>{{ $t('inspection_dashboard_expiry_date') }}</dt><dd>{{ formatDate(vehicle.inspectionExpiryDate) }}</dd></div></dl>

          <div v-if="vehicleRenewalStatus" class="inspection-renewal-status"><span>{{ $t('inspection_renewal_application_status') }}</span>

            <v-chip
              :class="['details-renewal-status', `details-renewal-status--${vehicleRenewalStatus.color}`]"
      ...
      <v-card class="details-card inspection-history-card" elevation="0"><div class="details-card__title"><v-avatar color="primary" icon="mdi-history" size="58" variant="tonal" /><h2>{{ $t('inspection_vehicle_inspection_history') }}</h2></div><v-divider class="my-4" /><div v-if="inspectionHistoryLoading" class="vehicle-history-state"><v-progress-circular color="primary" indeterminate size="24" /></div>

        <v-alert v-else-if="inspectionHistoryLoadError" density="compact" type="error" variant="tonal"><div class="d-flex flex-wrap align-center justify-space-between ga-2">{{ $t('inspection_history_load_error') }}<v-btn
          color="error"
          :disabled="inspectionHistoryLoading"
          :loading="inspectionHistoryLoading"
          size="small"
          variant="outlined"
          @click="retryInspectionHistory"
        >{{ $t('retry') }}</v-btn></div></v-alert>

        <v-alert v-else-if="vehicleInspectionHistory.length === 0" density="compact" type="info" variant="tonal">{{ $t('inspection_history_empty_title') }}</v-alert><div v-else class="vehicle-history-table-wrap"><table class="vehicle-history-table"><colgroup><col class="vehicle-history-reference-column"><col class="vehicle-history-date-column"><col class="vehicle-history-station-column"><col class="vehicle-history-result-column"></colgroup><thead><tr><th scope="col">{{ $t('inspection_history_application_reference') }}</th><th scope="col">{{ $t('inspection_history_date') }}</th><th scope="col">{{ $t('inspection_history_station') }}</th><th class="vehicle-history-result" scope="col">{{ $t('inspection_history_result') }}</th></tr></thead><tbody><tr v-for="inspection in vehicleInspectionHistory" :key="`${inspection.applicationId}-${inspection.attemptNumber}-${inspection.inspectedAt}`"><td class="vehicle-history-reference">{{ inspection.referenceNumber || '—' }}</td><td>{{ formatDate(inspection.inspectedAt) }}</td><td class="vehicle-history-station">{{ inspectionStationName(inspection) }}</td><td class="vehicle-history-result"><v-chip :color="inspectionResultColor(inspection)" :prepend-icon="inspectionResultIcon(inspection)" size="small" variant="tonal">{{ inspectionResultLabel(inspection) }}</v-chip></td></tr></tbody></table></div>

        <div v-if="!inspectionHistoryLoading && !inspectionHistoryLoadError && vehicleInspectionHistory.length > 0" class="vehicle-history-mobile-list">
          <article v-for="inspection in vehicleInspectionHistory" :key="`${inspection.applicationId}-${inspection.attemptNumber}-${inspection.inspectedAt}`" class="vehicle-history-mobile-record">
            <div class="vehicle-history-mobile-record__reference"><span>{{ $t('inspection_history_application_reference') }}</span><strong>{{ inspection.referenceNumber || '—' }}</strong></div>
            <div class="vehicle-history-mobile-record__date"><span>{{ $t('inspection_history_date') }}</span><strong>{{ formatDate(inspection.inspectedAt) }}</strong></div>
            <div class="vehicle-history-mobile-record__result"><span>{{ $t('inspection_history_result') }}</span><v-chip :color="inspectionResultColor(inspection)" :prepend-icon="inspectionResultIcon(inspection)" size="small" variant="tonal">{{ inspectionResultLabel(inspection) }}</v-chip></div>
            <div class="vehicle-history-mobile-record__station"><span>{{ $t('inspection_history_station') }}</span><strong>{{ inspectionStationName(inspection) }}</strong></div>
          </article>
        </div>
...
<style scoped>
  .vehicle-details-view :deep(*) { font-weight: 400 !important; }
  .vehicle-details-view { padding-bottom: 20px; }.details-topline { align-items: center; display: flex; justify-content: space-between; min-height: 32px; }.details-breadcrumbs :deep(.v-breadcrumbs-item--link) { color: #697080; }.details-breadcrumbs :deep(.details-breadcrumbs__current) { background: #d8def8; border-radius: 999px; color: #2a3472; font-weight: 700; padding: 4px 10px; }.back-action { color: #2a3472; font-weight: 800; }.details-heading { margin: 10px 0 17px; }.details-heading h1 { color: #102652; font-size: clamp(1.35rem, 2vw, 1.72rem); font-weight: 800; line-height: 1.2; }.details-heading p { align-items: center; color: #20212a; display: flex; flex-wrap: wrap; font-size: .94rem; font-weight: 600; gap: 6px; line-height: 1.45; margin-top: 3px; }.details-plate-label { color: #3a3b43; font: inherit; font-weight: 700; }.details-state { display: grid; min-height: 380px; place-items: center; }
  .details-summary-grid { align-items: start; display: grid; gap: 20px; grid-template-columns: minmax(0, 2fr) minmax(0, 1fr); }.details-card { border: 1px solid #eff0f4; border-radius: 12px; box-shadow: 0 4px 13px rgba(32, 38, 71, .045); padding: 16px; }.details-card__title { align-items: center; display: flex; gap: 9px; }.details-card__title :deep(.v-avatar) { height: 42px !important; width: 42px !important; }.details-card__title h2 { color: #2a3472; font-size: 1.05rem; font-weight: 800; line-height: 1.3; }.details-grid, .owner-grid { display: grid; gap: 15px 24px; grid-template-columns: repeat(2, minmax(0, 1fr)); }.details-grid dt, .owner-grid dt { color: #282a33; font-size: .82rem; font-weight: 600; margin-bottom: 3px; }.details-grid dd, .owner-grid dd { color: #282a33; font-size: .94rem; font-weight: 800; line-height: 1.45; margin: 0; overflow-wrap: anywhere; }
  .inspection-status-card { background: #293675; border-radius: 12px; box-shadow: 0 5px 13px rgba(41, 54, 117, .16); color: white; padding: 16px; }.inspection-status-card h2 { font-size: .96rem; font-weight: 800; }.inspection-status-card :deep(.v-chip) { font-size: .82rem; font-weight: 800; }.details-inspection-status--expired { background: #fee2e2 !important; color: #b42318 !important; }.details-inspection-status--expiring { background: #fff1cf !important; color: #8a4b00 !important; }.details-inspection-status--valid { background: #dcfce7 !important; color: #166534 !important; }.inspection-status-card dl { margin: 11px 0; }.inspection-status-card dl div { align-items: center; border-bottom: 1px solid rgba(255,255,255,.14); display: flex; gap: 8px; justify-content: space-between; padding: 8px 0; }.inspection-status-card dt { color: #c1caef; font-size: .86rem; font-weight: 600; }.inspection-status-card dd { font-size: .92rem; font-weight: 800; margin: 0; text-align: right; }.days-remaining { display: block; font-size: 1.15rem; line-height: 1.1; text-align: center; }.days-remaining--expired { color: #ffb4b8; }.days-remaining--expiring { color: #ffda87; }.inspection-status-card > span { color: #c7cff0; display: block; font-size: .76rem; margin-top: 5px; text-align: center; }
  .inspection-renewal-status { align-items: center; border-bottom: 1px solid rgba(255,255,255,.14); display: flex; gap: 8px; justify-content: space-between; margin-bottom: 12px; padding: 2px 0 12px; }.inspection-renewal-status > span { color: #c1caef; font-size: .78rem; font-weight: 600; }.inspection-renewal-status :deep(.v-chip) { flex: 0 0 auto; }.details-renewal-status--info { background: #dbeeff !important; color: #075985 !important; }.details-renewal-status--deep-purple { background: #ede9fe !important; color: #5b21b6 !important; }.details-renewal-status--secondary { background: #f0e7ff !important; color: #6b21a8 !important; }.details-renewal-status--success { background: #dcfce7 !important; color: #166534 !important; }.details-renewal-status--warning { background: #fff1cf !important; color: #8a4b00 !important; }.details-renewal-status--error { background: #fee2e2 !important; color: #b42318 !important; }
  .owner-card, .inspection-history-card { margin-top: 13px; }.owner-grid { grid-template-columns: 1.05fr 1.1fr 1fr 1.2fr; }.vehicle-history-state { display: grid; min-height: 56px; place-items: center; }.vehicle-history-table-wrap { overflow-x: auto; }.vehicle-history-table { border-collapse: collapse; table-layout: fixed; width: 100%; }.vehicle-history-date-column { width: 16%; }.vehicle-history-reference-column { width: 25%; }.vehicle-history-station-column { width: 44%; }.vehicle-history-result-column { width: 15%; }.vehicle-history-table th { background: #fafafd; color: #6c6e79; font-size: .7rem; font-weight: 800; padding: 10px 12px; text-align: left; white-space: nowrap; }.vehicle-history-table td { border-bottom: 1px solid #e0e1e7; color: #30323c; font-size: .79rem; font-weight: 700; padding: 13px 12px; vertical-align: middle; }.vehicle-history-table tbody tr:last-child td { border-bottom: 0; }.vehicle-history-station { color: #535661 !important; font-weight: 600 !important; overflow-wrap: anywhere; }.vehicle-history-reference { color: #273d7a !important; font-family: 'Siemreap', sans-serif; font-size: .75rem !important; overflow-wrap: anywhere; }.vehicle-history-result { text-align: right !important; white-space: nowrap; }.vehicle-history-footer { border-top: 1px solid #e0e1e7; display: flex; justify-content: flex-end; margin-top: 2px; padding-top: 6px; }.vehicle-history-footer :deep(.v-btn) { font-size: .76rem; font-weight: 800; letter-spacing: 0; }.details-actions { border-top: 1px solid #dedfe6; display: flex; flex-wrap: wrap; gap: 8px; margin-top: 18px; padding-top: 14px; }.details-actions :deep(.v-btn) { font-size: .78rem; font-weight: 800; letter-spacing: 0; min-height: 34px; }
  @media (max-width: 1279px) { .details-summary-grid { grid-template-columns: 1fr; }.owner-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }@media (max-width: 900px) { .vehicle-history-table, .vehicle-history-table tbody { display: block; width: 100%; }.vehicle-history-table thead { display: none; }.vehicle-history-table tr { display: grid; gap: 7px 12px; grid-template-areas: 'date result' 'reference reference' 'station station'; grid-template-columns: minmax(0, 1fr) auto; padding: 13px 0; }.vehicle-history-table td { border: 0; min-width: 0; padding: 0; }.vehicle-history-table tbody tr + tr { border-top: 1px solid #e0e1e7; }.vehicle-history-table td:nth-child(1) { grid-area: date; }.vehicle-history-table td:nth-child(2) { grid-area: reference; }.vehicle-history-table td:nth-child(3) { grid-area: station; }.vehicle-history-table td:nth-child(4) { grid-area: result; }.vehicle-history-footer { justify-content: flex-start; } }@media (max-width: 600px) { .details-topline { align-items: flex-start; flex-direction: column; gap: 6px; }.details-heading { margin-bottom: 22px; }.details-card { border-radius: 14px; padding: 20px; }.details-grid, .owner-grid { gap: 22px; grid-template-columns: 1fr; }.details-actions { flex-direction: column; } }
  .inspection-status-card h2 { color: #c1caef; font-size: .92rem; }
  .inspection-status-card dt, .inspection-renewal-status > span { font-size: .86rem; }
  .inspection-status-card dd { font-size: .92rem; }
  .inspection-status-card > span { font-size: .8rem; }
  .inspection-status-card :deep(.v-chip) { font-size: .82rem; }
  .vehicle-details-view .inspection-status-card :deep(.v-chip),
  .vehicle-details-view .inspection-status-card :deep(.v-chip__content) { font-size: .82rem !important; }
  .details-breadcrumbs :deep(.v-breadcrumbs-item) { font-size: .94rem; }
  .back-action { font-size: .9rem; }
  .vehicle-history-station { font-weight: 400 !important; }
  .vehicle-history-table td:nth-child(2) { color: #2a3472 !important; }
  .vehicle-history-reference { color: #30323c !important; }
  .details-grid dt, .owner-grid dt { color: #5c6070; font-size: .86rem; }
  .vehicle-details-view .details-card .details-grid dt,
  .vehicle-details-view .details-card .owner-grid dt { font-size: .85rem !important; }
  .vehicle-details-view .inspection-status-card dl dt,
  .vehicle-details-view .inspection-status-card .inspection-renewal-status > span,
  .vehicle-details-view .inspection-history-card .vehicle-history-table th { font-size: .85rem !important; }
  .vehicle-history-mobile-list { display: none; }

  @media (max-width: 600px) {
    .details-grid, .owner-grid { gap: 14px; }
    .details-grid > div, .owner-grid > div { align-items: start; display: grid; gap: 12px; grid-template-columns: minmax(0, 1fr) minmax(0, 1.25fr); }
    .details-grid dt, .owner-grid dt { margin-bottom: 0; }
    .details-grid dd, .owner-grid dd { text-align: right; }
    .details-topline { align-items: center; flex-direction: row; flex-wrap: nowrap; gap: 8px; min-height: 0; }
    .details-breadcrumbs { flex: 1 1 auto; min-width: 0; overflow: visible; }
    .back-action { flex: 0 1 auto; font-size: .84rem !important; min-width: 0; padding-inline: 6px; }
    .back-action :deep(.v-btn__prepend) { margin-inline: 0 4px; }
    .back-action :deep(.v-icon) { font-size: 1.25rem !important; }
  }

  @media (max-width: 900px) {
    .vehicle-history-table-wrap { display: none; }
    .vehicle-history-mobile-list { display: grid; gap: 0; }
    .vehicle-history-mobile-record { display: grid; gap: 12px; grid-template-areas: 'reference date result' 'station station station'; grid-template-columns: minmax(0, 1.2fr) minmax(0, 1fr) auto; padding: 14px 0; }
    .vehicle-history-mobile-record + .vehicle-history-mobile-record { border-top: 1px solid #e0e1e7; }
    .vehicle-history-mobile-record > div { min-width: 0; }
    .vehicle-history-mobile-record__date { grid-area: date; }
    .vehicle-history-mobile-record__result { grid-area: result; text-align: right; }
    .vehicle-history-mobile-record__reference { grid-area: reference; text-align: center; }
    .vehicle-history-mobile-record__station { grid-area: station; }
    .vehicle-history-mobile-record span { color: #6c6e79; display: block; font-size: .82rem; margin-bottom: 3px; }
    .vehicle-history-mobile-record strong { color: #30323c; display: block; font-size: .94rem; font-weight: 400 !important; overflow-wrap: anywhere; }
    .vehicle-history-mobile-record__result :deep(.v-chip) { align-items: center; font-size: .84rem; gap: 2px; justify-content: center; padding-inline: 8px; }
    .vehicle-history-mobile-record__result :deep(.v-chip__prepend) { margin-inline: 0 2px !important; }
    .vehicle-history-mobile-record__result :deep(.v-chip__content) { margin-inline: 0 !important; }
    .vehicle-history-footer { justify-content: flex-end; }
  }
</style>
