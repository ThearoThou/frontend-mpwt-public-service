...
<template>
  ...
  <v-app-bar border color="surface" elevation="0" height="72">
    ...
    <v-app-bar-title class="text-subtitle-1 font-weight-bold">{{ $t('inspection_service_title') }}</v-app-bar-title>
    <v-menu>
      <template #activator="{ props }"><v-btn v-bind="props" append-icon="mdi-chevron-down" variant="text">{{ displayName }}</v-btn></template>
